...
1. 基于资源指标（如CPU、内存）
- 指标收集：
  - HPA控制器会定期从集群中的Metrics - Server（或其他配置的资源指标采集器）获取Pod的资源使用情况，包括CPU和内存等指标的使用数据。Metrics-Server会收集每个Pod的实时资源使用信息。
- 指标评估与决策：
  - 对于基于CPU利用率的缩放，例如，在HPA定义中设置了目标CPU利用率为50%。HPA控制器会计算所有Pod的平均CPU利用率。如果平均CPU利用率超过了设定的50%阈值，并且当前Pod副本数小于HPA中定义的最大副本数，HPA就会决定增加Pod副本数。
  - 假设当前有3个Pod，平均CPU利用率达到了60%，而HPA的最大副本数为10，那么HPA可能会决定增加Pod副本数，比如增加到4个，以分担负载，降低每个Pod的CPU使用率。相反，如果平均CPU利用率低于阈值且副本数大于最小副本数，HPA会减少副本数。例如，当平均CPU利用率下降到30%，而最小副本数为2，HPA可能会将Pod副本数减少到2个，以避免资源浪费。
2. 基于自定义指标（应用特定指标）
- 自定义指标提供：
  - 如果要根据应用程序自定义的指标（如每秒请求数、队列长度等）进行缩放，需要先在集群中实现自定义指标API。通常会部署如Prometheus等提供自定义指标的服务，并将其与Kubernetes集成，使HPA控制器能够获取这些自定义指标的值。
- 缩放操作触发：
  - 例如，定义了一个自定义指标 http_requests_per_second（每秒请求数），在HPA资源定义中设置了目标值为100（即当平均每秒请求数达到100时进行缩放）。HPA控制器会从自定义指标API获取该指标的值，当该值达到或超过设定目标时，如果当前副本数未达到最大限制，HPA会增加Pod副本数；如果该值低于设定目标且副本数大于最小限制，HPA会减少副本数。
3. 基于外部指标（集群外部系统提供的指标）
- 外部指标引入：
  - 类似于自定义指标，需要配置集群以支持获取外部指标。这可能涉及设置相关的适配器或服务，将外部系统（如云服务提供商的监控服务）的指标引入到Kubernetes集群中，使HPA控制器能够访问这些指标。
- 根据外部指标缩放：
  - 例如，如果使用云服务提供商的特定指标（如AWS的特定负载均衡器的请求数等），在HPA定义中引用该外部指标。当外部指标的值满足HPA中设定的缩放条件（如达到一定的请求数阈值）时，HPA会相应地调整Pod副本数，以适应外部系统的负载变化。
...
- 该技术涉及定义一些规则，这些规则会根据特定性能指标（如CPU使用率）的预定义阈值来触发伸缩操作。
...
- QT通常在分析阶段用于估算性能指标，如队列长度和请求的平均等待时间。
- 它涉及将系统建模为由队列和服务器组成的网络，然后运用数学公式或模拟来预测其行为。
- QT为理解系统性能提供了理论基础，但在处理具有动态工作负载和多种资源类型的复杂场景时可能会存在困难。

=== Control Theory (CT)

- 控制论通过调整资源分配以维持期望的性能水平（例如使CPU负载接近目标值），将控制系统应用于自动伸缩过程。
- 可以使用不同类型的控制器，从较为简单的固定增益控制器（如比例-积分-微分控制器，PID）到更为复杂的自适应控制器（如模糊控制器和模型预测控制器）不等。
- CT为自动伸缩提供了一个稳健的框架，但需要对应用程序进行仔细建模，并且对模型的不准确性较为敏感。
...
文中提到的分解方法会将时间序列分解为trend、seasonality和residual terms，它们的作用如下：
...
- seasonality
  - seasonality代表了数据中重复出现的周期性波动模式。这些波动通常与时间相关，例如按天、按周、按月等周期性循环。
...
@matheq2 是用于预测下一个时刻$y_(t+1)$的数据值。
$
...
- 适用场景
  - M/M/1：当Pod数量相对较少，且业务请求量相对较小时，M/M/1模型可能表现较好，因为此时单个Pod足以应对请求，且模型简单易于理解和计算。在实际应用中，如果业务的并发度不高，资源需求相对稳定，M/M/1模型可以提供较为准确的性能评估和资源配置建议。
  - M/M/c：当业务请求量较大，且Pod数量可灵活调整时，M/M/c模型更具优势。它能够充分利用多个服务台的并行处理能力，有效降低请求的平均等待时间，提高系统的整体性能。例如在一些大型互联网应用中，面对高并发的用户请求，采用M/M/c模型可以更好地优化资源配置，提升用户体验。
...
- 动态资源调整
  - 准确预测业务需求：使用基于稳健分解的统计方法（如RobustPeriod、RobustSTL、RobustTrend等算法）以及深度学习模型（如FEDformer、Quatformer等，适用于数据充足场景）对未来业务进行预测，能够提前感知业务变化趋势，解决了现有方法无法动态调整POD资源以适应业务波动的问题，避免了资源浪费或不足的情况。
- 优化资源分配
  - 性能模型训练：通过采用排队理论中的不同模型（如线性关系的并行M/M/1队列和具有公共缓冲pod的M/M/c队列），根据业务QPS、Pod数量、Pod CPU使用率等指标，模拟指标度量与所需Pod数量之间的关系，找到满足平均响应时间（RT）要求的最小Pod数量，从而优化资源分配，提高资源利用率。
- 自动化与智能化
  - 自动生成缩放计划：根据预测的工作量和性能模型，自动生成包括Pod数量和增减时间的缩放计划，无需像CronHPA那样依赖人工手动设置，提高了调整的准确性和灵活性，节省了运维成本。
